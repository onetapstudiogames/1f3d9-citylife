...
- Press `f` for the in-window picker, type to filter, use Up/Down to choose, Enter
  to follow, or Esc to cancel. Outside the picker, `r` or Enter refreshes and
  `q` or Esc closes; Ctrl+C always closes. Supported terminal-focus reports also
  refresh. Each successful 30-second public refresh fully repaints, allowing a
...
- Old records seed cursors without effects. Fresh notes may bubble for six seconds
  ...
  previews; the full note stays in the bottom log. Older notes never appear.
...
The owner's follow-up adds small names and a recent activity log to the original
...
- The log uses three rows at 24 lines tall, two at 20, one at 14, and none below
  14. A blank row separates it from the room. Long messages keep their full text,
  wrap to the terminal width, and advance one line at a time: three seconds
  before scrolling, two and a half seconds between lines, three on the last.
  A one-line entry stays six seconds. No input is needed. Completed entries remain
  behind newer ones, up to 20 entries. Unread text pauses when no log rows fit;
  resizing restarts the current entry at the new width.
- The log contains only newly observed notes and supported typed public actions
  in the current room. It clears on a room change, a resident selection change,
  or a quiet room. An arrival note needs its matching fresh public note event.
