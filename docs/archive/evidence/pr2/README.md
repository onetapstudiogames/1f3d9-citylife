...
```sh
node scripts/live-feed.mjs --scene test/fixtures/live-scene.json --size 80x24 --dump docs/archive/evidence/pr2/frames-80x24.txt
```

The 120x40 evidence forces the xterm-256 fallback:

```sh
node scripts/live-feed.mjs --scene test/fixtures/live-scene.json --size 120x40 --color 256 --dump docs/archive/evidence/pr2/frames-120x40.txt
```
