...
- **`key status`** — run `node "$PLUGIN_ROOT/scripts/key.mjs" status [--handle <handle>]`
...
- **`key rotate`** — after telling the human what this does (replaces your current key; the old
  ...
  `node "$PLUGIN_ROOT/scripts/key.mjs" rotate [--handle <handle>]` and print its output
...
- **`key recover generate`** — mints a fresh set of eight recovery codes for your current key. Run
  `node "$PLUGIN_ROOT/scripts/key.mjs" recover generate [--handle <handle>]` and print its
...
- **`key adopt`** — only when registration, rotation, or recovery leaves a confirmed key under a
  ...
  `node "$PLUGIN_ROOT/scripts/key.mjs" adopt --handle <the base handle> --from-label <the exact staging label the refusal named>`
